...
func TestDeduplicator_Wrap_SkipsDuplicate(t *testing.T) {
	...
	evt := &events.Event{ID: "wrap-dup", CorrelationID: "corr-1", Type: events.InventoryReserved}
...
func TestDeduplicator_Wrap_UnmarksOnError(t *testing.T) {
	...
	wrapped := c.Wrap(handler)
	evt := &events.Event{ID: "wrap-retry", CorrelationID: "corr-2", Type: events.PaymentFailed}
...
func TestDeduplicator_Wrap_KeepsMarkOnSuccess(t *testing.T) {
	...
	wrapped := c.Wrap(handler)
	evt := &events.Event{ID: "wrap-ok", CorrelationID: "corr-3", Type: events.PaymentRolledBack}
